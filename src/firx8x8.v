...
// use 24 bits for the Accumulators. 
	...
	fir256 #(.BEGIN_COEFF(11'd0)   , .ABITS(ABITS), .TAPS(TAPS)) A (clock, waddr, weA, x_real, x_imag, RaccA, IaccA);
	fir256 #(.BEGIN_COEFF(11'd256) , .ABITS(ABITS), .TAPS(TAPS)) B (clock, waddr, weB, x_real, x_imag, RaccB, IaccB);
	fir256 #(.BEGIN_COEFF(11'd512) , .ABITS(ABITS), .TAPS(TAPS)) C (clock, waddr, weC, x_real, x_imag, RaccC, IaccC);
	fir256 #(.BEGIN_COEFF(11'd768) , .ABITS(ABITS), .TAPS(TAPS)) D (clock, waddr, weD, x_real, x_imag, RaccD, IaccD);
	fir256 #(.BEGIN_COEFF(11'd1024), .ABITS(ABITS), .TAPS(TAPS)) E (clock, waddr, weE, x_real, x_imag, RaccE, IaccE);
	fir256 #(.BEGIN_COEFF(11'd1280), .ABITS(ABITS), .TAPS(TAPS)) F (clock, waddr, weF, x_real, x_imag, RaccF, IaccF);
	fir256 #(.BEGIN_COEFF(11'd1536), .ABITS(ABITS), .TAPS(TAPS)) G (clock, waddr, weG, x_real, x_imag, RaccG, IaccG);
	fir256 #(.BEGIN_COEFF(11'd1792), .ABITS(ABITS), .TAPS(TAPS)) H (clock, waddr, weH, x_real, x_imag, RaccH, IaccH);
...
module fir256(
	...
	parameter BEGIN_COEFF	= 0;							// ROM coefficients
    ...
    Gowin_pROM_2048 fir2048(// coefficient  8 banks ROM  18 X 256  (in file "fir_8x8.mi")
        .dout(coef), //output [17:0] dout
        .clk(clock), //input clk
        .oce(1'b1), //input oce
        .ce(1'b1), //input ce
        .reset(1'b0), //input reset
        .ad(BEGIN_COEFF+caddr) //input [10:0] ad
    );
