...
// use 24 bits for the Accumulators. 
   ...
   assign weB 		= (x_avail && wstate == 1);
	...
	fir256d #(.BEGIN_COEFF(8'd0),   .ABITS(ABITS), .TAPS(TAPS)) 
        A (clock, waddr, raddr, weA, x_real, x_imag, RaccAa, IaccAa);					// first bank odd coeff
	fir256d #(.BEGIN_COEFF(8'd64),  .ABITS(ABITS), .TAPS(TAPS)) 
        B (clock, (waddr + 8'd64), raddr, weA, x_real, x_imag, RaccAb, IaccAb);	// second bank
	fir256d #(.BEGIN_COEFF(8'd128), .ABITS(ABITS), .TAPS(TAPS)) 
        C (clock, waddr, raddr, weB, x_real, x_imag, RaccBa, IaccBa);  				// first bank even coeff
	fir256d #(.BEGIN_COEFF(8'd192), .ABITS(ABITS), .TAPS(TAPS)) 
        D (clock, (waddr + 8'd64), raddr, weB, x_real, x_imag, RaccBb, IaccBb); 	// second bank
...
module fir256d(
   ...
   reg [ADDRBITS-1:0] read_address;	
    ...
    Gowin_pROM rom256(
        ...
        .ad(BEGIN_COEFF+caddr) //input [7:0] ad
    );

    ram48x256 ram48(   	// sample RAM 48 X 256;  48 bit = 24 bits I and 24 bits Q
     .clock(clock),
